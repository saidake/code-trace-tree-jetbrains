...
| Global XML | `<OS Config Dir>/code-trace-tree/<projectId>.xml` (path match picks latest `updatedAt` when several XMLs share the same `<path>`; legacy `FolderName.xml` still resolved by scanning `<projectId>`). Skip `settings.xml` when scanning. |
