...
`isValid` is never stored. Recomputed on load/reload, when a file is opened (LINE nodes in that file, against the editor document), and when the user clicks **Recheck Trace Availability** (all LINE / FILE / DIRECTORY nodes):
