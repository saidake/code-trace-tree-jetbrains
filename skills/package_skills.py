...
def detect_version(explicit: str | None) -> str:
    ...
    gradle = REPO_ROOT / "main" / "build.gradle.kts"
    text = gradle.read_text(encoding="utf-8")
    m = re.search(r'val pluginVersion\s*=\s*"([^"]+)"', text)
    if not m:
        raise SystemExit("Could not detect pluginVersion from main/build.gradle.kts")
    return m.group(1)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--version", help="Skill/package version (default: pluginVersion)")
    parser.add_argument(
        "--agents",
